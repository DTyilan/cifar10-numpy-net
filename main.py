...
def train(model, X_train, y_train, X_val, y_val,
    ...
    for it in range(num_iters):
        ...
        _, probs = model.loss(X_batch,y_batch,reg)
        grads = model.backward(y_batch,probs, reg)

        model.params = sgd_update(model.params, grads, learning_rate)

        if it % 100 == 0:
            train_loss, _ = model.loss(X_train, y_train, reg)
            ...
            print('iteration %d / %d: train loss %.4f, val loss = %.4f, val acc = %.2f%%' %
                  (it, num_iters, train_loss, val_loss, val_acc * 100))
    ...
    plt.subplot(1, 2, 1)
    plt.plot([i * 100 for i in range(len(train_loss_history))],
         train_loss_history, label='Train Loss')
    plt.plot([i * 100 for i in range(len(val_loss_history))],
    ...
    plt.subplot(1, 2, 2)
    plt.plot([i * 100 for i in range(len(val_acc_history))],
    ...
    hyperparams_text = f"Learning Rate: {raw_lr}\n" \
                   f"Hidden Size 1: {model.params['W1'].shape[1]}\n" \
                   f"Hidden Size 2: {model.params['W2'].shape[1]}\n" \
                   f"Regularization: {reg}"
    plt.figtext(0.01, 0.95, hyperparams_text, ha="left", va="top",
    ...
    filename = f'{save_dir}/lr_{raw_lr}_hs1_{model.params["W1"].shape[1]}_hs2_{model.params["W2"].shape[1]}_reg_{reg}.png'
...
def hyperparameter_search(X_train, y_train, X_val, y_val,
    ...
    for lr in learning_rates:
        for hs1 in hidden_size1:
            for hs2 in hidden_size2:
                for reg in reg_strengths:
                    model = MLP(input_size=32 * 32 * 3,
                             hidden_size1=hs1, hidden_size2=hs2,output_size=10)
                    model = train(model, X_train, y_train, X_val, y_val,
                              learning_rate=lr, reg=reg, num_iters=num_iters)
                    val_acc = (predict(model, X_val) == y_val).mean()
                    print('lr %e, hidden_size1 %d,hidden_size2 %d reg %e, val acc: %.2f%%' %
                      (lr, hs1, hs2, reg, val_acc * 100))
                    if val_acc > best_val_acc:
                        best_val_acc = val_acc
                        best_hparams = {'learning_rate': lr,
                                    'hidden_size1': hs1,'hidden_size2':hs2, 'reg': reg}
...
def test(model, X_test, y_test, model_path):
    if model_path:
...
if __name__ == "__main__":
    ...
    hidden_size1 = [512, 256]
    hidden_size2 = [64, 128]
    reg_strengths = [5e-3, 1e-3]
    num_iters = 3000
    ...
    # 使用最佳超参数训练模型并保存参数
    best_model = MLP(input_size=32 * 32 * 3,
                     hidden_size1=best_hparams['hidden_size1'],
                     hidden_size2=best_hparams['hidden_size2'],
                       output_size=10)
